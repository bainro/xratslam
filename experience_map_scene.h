...
public:
 ExperienceMapScene(ptree & settings, ExperienceMap *in_map)
    ...
    map(in_map)
  {
    ...
    // Debug.
    std::ofstream file( "EMSettings.xml" );
    write_xml( file, settings,
    ...
    irr::scene::ICameraSceneNode *camera =
      scene->addCameraSceneNode( NULL,
                                 irr::core::vector3df((irr::f32)(0.5 * width),
                                                      (irr::f32)(0.5 * height),
                                                      -10 ),
                                 irr::core::vector3df((irr::f32)(0.5 * width),
                                                      (irr::f32)(0.5 * height),
    ...
    proj.buildProjectionMatrixOrthoLH( (irr::f32)width, (irr::f32)height,
    ...
    irr::video::ITexture *irat_texture =
      exp_map_scene->getVideoDriver()->
      getTexture( (media_path + "/" + image_file).c_str() );
    if (irat_texture == 0)
    {
      std::cout
        << "WARNING: Unable to load texture: "
    ...
    if (irat_texture)
    {
      cout <<"ExperienceMapScene() - irat_texture."<< endl;
      irat_node =
        exp_map_scene->addMeshSceneNode(
    ...
    {
      cout <<"ExperienceMapScene() - NOT irat_texture."<< endl;
    ...
    if (irat_texture)
    {
      cout <<"ExperienceMapScene() - irat_texture 2."<< endl;
    ...
    cout <<"ExperienceMapScene() - END."<< endl;
  }
  ...
  void update_experiences()
  {
    ...
    /*
    cout << "ExperienceMapScene::update_experiences() - numexps: "
         << map->experiences.size()
         << endl;
    */
  ...
  void update_current_experience(int exp)
  {
    ...
    /*
    cout
      << "update_current_experience(): "<< endl
      << "  x2d1: "<< x2d1 << endl
      << "  y2d1: "<< y2d1 << endl
      ;
    */
  ...
  void update_scene()
  {
    ...
    //    cout << "ExperienceMapScene::update_scene() - numexps: "<< numexps << endl;

    // Find experiences "max" and "min" positions.
    update_experiences();
